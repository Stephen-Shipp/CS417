# Requirements
  ...
  * No external dependencies (uses only the `math` module from the standard library)
...
```
python3 finite_difference.py
```

There are no command line arguments. All values and functions are hardcoded:
  * f(x) = sin(x)
  * x = 1
...
The test suite can be run using the provided shell script:
```
./run_tests.sh
```
